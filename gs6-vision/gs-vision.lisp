...
;;; gs-vision.lisp (gs6-vision) -- the module itself: creation, reset,
;;; requests, queries.  Identical to the gs-vision file except for the extra
;;; GS6 state that a reset clears, the default stop policy (adaptive: the GS6
;;; quit signal alone) and the start-point offset in gs-state.
;;;
...
subject, so those have to survive between trials (handoff section 11)."
  ...
  (bt:with-recursive-lock-held ((gs-lock vis-mod))
    ...
    (setf (select-event vis-mod) nil
          ...
          (start-offset vis-mod) 0.0
          (search-start-point vis-mod) 0.0
    ...
    (if s
        (let ((v (spec-slot-value s)))
          (if (member v '(adaptive cgs both)) v
            (progn (print-warning "gs-search stop must be adaptive, cgs or both, not ~s. Using adaptive." v)
                   'adaptive)))
      'adaptive)))
...
(defun gs-state-command ()
  "Adaptive state, for logging: (quit-threshold prevalence n-feedbacks start-offset)."
  (let ((v (get-module :vision)))
    (when (typep v 'gs-vision-module)
...
(dolist (c '(("gs-fixation-log" gs-fixation-log-command
             ...
             ("gs-state" gs-state-command
              "Adaptive state as (quit-threshold prevalence n-feedbacks start-offset). No params.")))
  ...
  (if (mp-models)
    ...
    (let ((stock (gs-capture-vision-parameters)))
      ...
      (define-module-fct :vision
        ...
        :documentation "Guided Search 6 vision module (GS6 diffuser and quit signal + GS2 channels + PAAV + EMMA)"
