...
;;; gs-eye.lisp (gs6-vision) -- acuity, iconic memory, functional visual fields, saccades.
;;;
;;; Identical to the gs-vision file except that GS-REFRESH-ICONIC also stores
;;; the Guided Search 2 left/right (TILT) channel when :gs-orient-dual is on.
;;;
                  ...
                  (when v
                    (setf ch (append ch (list slot
                                              (gs-channels-for vis-mod slot v
                                                               (getf raw 'hue))))))))
              ;; Guided Search 2: the left/right channel is a second
              ;; dimension driven by the same orientation feature.
              (when (and (orient-dual vis-mod) (getf raw 'orient))
                (setf ch (append ch (list 'tilt (gs-tilt-channels (getf raw 'orient))))))
